 ...
 * agricultural crop itineraries for Indian farming when external LLMs are offline or timing out.
 */

// ======================================================
// Crop Knowledge Base Templates
// ======================================================
...
const CROP_KNOWLEDGE_BASE = {
  wheat: {
    ...
    yieldPerAcre: 22, // Quintals per acre base
    landPrep: [
  ...
  sugarcane: {
    ...
    yieldPerAcre: 350, // Quintals / Tonnes equivalent
    landPrep: [
    ...
    seed: {
      ...
      baseSeedQtyKgPerAcre: 30000, // 3-bud setts count
      costPerAcre: 8500
    },
    timeline: [
  ...
  cotton: {
    ...
    yieldPerAcre: 12, // Quintals per acre base
    landPrep: [
  ...
  groundnut: {
    ...
    seed: {
      ...
      baseSeedQtyKgPerAcre: 45, // Kernels
      costPerAcre: 4500
    },
    timeline: [
  ...
  tomato: {
    ...
    seed: {
      ...
      baseSeedQtyKgPerAcre: 0.05, // 50 grams hybrid seed
      costPerAcre: 3500
    },
    timeline: [
  ...
  potato: {
    ...
    seed: {
      ...
      baseSeedQtyKgPerAcre: 1000, // Seed Tubers (10 Quintals)
      costPerAcre: 18000
    },
    timeline: [
...
// ======================================================
// Helper: Map Input String to Recognized Crop
// ======================================================

function getNormalizedCropKey(rawCropStr) {
  if (!rawCropStr) return "wheat";
  const c = rawCropStr.toLowerCase().trim();

  if (c.includes("wheat") || c.includes("gehu") || c.includes("gehun")) return "wheat";
  if (c.includes("sugarcane") || c.includes("ganna")) return "sugarcane";
  if (c.includes("cotton") || c.includes("kapas")) return "cotton";
  if (c.includes("rice") || c.includes("paddy") || c.includes("chawal") || c.includes("dhan")) return "rice";
  if (c.includes("maize") || c.includes("makka") || c.includes("corn")) return "maize";
  if (c.includes("soybean") || c.includes("soyabean")) return "soybean";
  if (c.includes("groundnut") || c.includes("mungfali") || c.includes("shengdana") || c.includes("peanut")) return "groundnut";
  if (c.includes("tomato") || c.includes("tamatar")) return "tomato";
  if (c.includes("onion") || c.includes("pyaz") || c.includes("kanda")) return "onion";
  if (c.includes("potato") || c.includes("aloo") || c.includes("batata")) return "potato";
...
function generateFallbackItinerary({
  ...
  // Economics computation ($Cost = Budget$, $Income = Budget \times 2$, $Profit = Income - Cost$)
  ...
  const yieldUnit = cropKey === "sugarcane" ? "Tonnes" : "Quintals";
  const expectedYieldStr = `${totalYieldMin} - ${totalYieldMax} ${yieldUnit} (Total for ${numArea} Acre${numArea > 1 ? "s" : ""})`;
  ...
  const seedQtyUnit = cropKey === "tomato" ? "grams" : "kg";
  ...
  console.log(`⚡ Instant Fallback Engine Generated Itinerary in ${latencyMs}ms for crop: "${itineraryPayload.crop}"`);
